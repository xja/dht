	...
	go func() {
		for resp := range w.Response() {
			...
			if err == nil {
				...
				con, err := sql.Open("mymysql", "magnet" + "/" + "user" + "/" + "pass")
                if err != nil {
                        panic(err)
                    }
                    // runs before return
                ...
                if id == 0 {
                    ...
                    if len(bt.Files) == 0 {
                        //_, err := 
						con.Exec("insert into hash_table (infohash, name, length, len_h, date, file_count) values (?, ?, ?, ?, ?, ?)", bt.InfoHash, bt.Name, bt.Length, len_h_calc(uint64(bt.Length)), time.Now().Format("2006-01-02"), 1)
						// fmt.Println(err)
                    } else {
						_, err := con.Exec("insert into hash_table (infohash, name, length, files, file_count, len_h, date) values (?, ?, ?, ?, ?, ?, ?)", bt.InfoHash, bt.Name, total_length, files_json, len(bt.Files), len_h_calc(total_length), time.Now().Format("2006-01-02"))
						fmt.Println(err)
                    }
					fmt.Println(bt.Name + "\n\n")
                }
